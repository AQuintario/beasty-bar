...
n_games = 10000
try:
    with open('ngames'+str(n_games)+'.pkl', 'rb') as log_file:
...
hl_size = 1000
try:
    with open('trained_nn_' + str(hl_size) + '_hlu_' + str(n_games) + '_games.pkl', 'rb') as nn_file:
    ...
    tic = time.clock()
    clf.fit(X, Y)
    with open('trained_nn_' + str(hl_size) + '_hlu_' + str(n_games) + '_games.pkl', 'wb') as nn_file:
...
for g in range(n_games):
    table = Table()
    players = [Player('Blue'), Player('Green', 'NN')]
    winner_color = play_a_game(table, players, logNN)
    wins[winner_color] += 1
